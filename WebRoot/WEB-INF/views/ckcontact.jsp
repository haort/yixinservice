	...
	<head>
		...
		<script type="text/javascript">
		 function repContact(id,rep){
			...
			$.ajax({     
		        type: 'post',     
		        url: "<%=request.getContextPath()%>/service/findContactsByOwner.do?wxId=${wxId}",
		        cache: false,       
		        dataType: 'json',     
		        success: function(data){ 
		        	var contact="";   
		            jQuery.each(data.contacts, function(i,item){
						...
						if(item.flag == "已解决"||item.flag == "提交上级部门"){
						  contact+="<p class='comment'>";
			            	contact+="<p><a href='javascript:void(0)' onclick='repContact("+item.id+",\"非常满意\")' class='buttonWrapper buttonOrange'>非常满意</a> ";
			            	contact+="<a href='javascript:void(0)' onclick='repContact("+item.id+",\"比较满意\")' class='buttonWrapper buttonDefault'>比较满意</a></p>";
			            	contact+="<p><a href='javascript:void(0)' onclick='repContact("+item.id+",\"服务一般\")' class='buttonWrapper buttonBlue'>服务一般</a> ";
			            	contact+="<a href='javascript:void(0)' onclick='repContact("+item.id+",\"不太满意\")' class='buttonWrapper buttonRed'>不太满意</a></p>";
			            	contact+="</p>";
				          contact+="</div>";	
			          }
			          contact+="</div>";
